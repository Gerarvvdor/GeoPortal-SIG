...
import { Plus, Edit2, Trash2, Search, MapPin, Phone, Clock, Building2, Save, X, Eye, Filter } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { MedicalCenter } from '../../types';
import { LoadingSpinner } from '../LoadingSpinner';
import { ErrorMessage } from '../ErrorMessage';

interface ClinicMedicProps {
  user: any;
}

export const ClinicMedic: React.FC<ClinicMedicProps> = ({ user }) => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('¿Estás seguro de que quieres eliminar esta clínica médica?')) {
      return;
    }
  ...
  if (loading && centers.length === 0) {
    return <LoadingSpinner size="lg" text="Cargando clínicas médicas..." />;
  }
    ...
    <div className="flex-1 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Gestión de Clínicas Médicas</h1>
            <p className="text-gray-600 mt-1 text-sm sm:text-base">Administra las clínicas comunales y centros especializados</p>
          </div>
          <button
            onClick={() => {
              setShowAddForm(true);
            ...
            className="bg-blue-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 shadow-sm text-sm sm:text-base"
          >
            <Plus className="w-4 h-4 sm:w-5 sm:h-5" />
            <span className="hidden sm:inline">Agregar Clínica Médica</span>
            <span className="sm:hidden">Nueva Clínica</span>
          </button>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-6 lg:p-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div className="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
              <div className="relative flex-1 sm:flex-none">
                ...
                <input
                  type="text"
                  placeholder="Buscar clínicas médicas..."
                  value={searchTerm}
            ...
            <div className="text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded-lg text-center sm:text-left">
              Total: <span className="font-semibold">{filteredCenters.length}</span> clínicas médicas
            </div>
          </div>
        </div>
        ...
        {filteredCenters.length === 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No hay clínicas médicas</h3>
            <p className="text-gray-500">Comienza agregando una nueva clínica médica.</p>
          </div>
        )}
      </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900">
                  {editingCenter ? 'Editar Clínica Médica' : 'Nueva Clínica Médica'}
                </h2>
                <button
                  onClick={() => {
                    setShowAddForm(false);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      placeholder="Ej: Clínica Médica San Salvador"
                    />
                  </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="Medicina General, Especialidades, Laboratorio, Rayos X"
                    />
                  </div>
